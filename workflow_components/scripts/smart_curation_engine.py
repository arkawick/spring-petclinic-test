...
class SmartCurationEngine:
    ...
    def load_npm_results(self, npm_json: str):
        """Load npm registry license fetch results"""
        try:
            with open(npm_json, 'r') as f:
                data = json.load(f)

            packages = data.get('packages', [])
            print(f"✅ Loaded npm results: {len(packages)} packages")

            for pkg in packages:
                pkg_id = pkg.get('id', '')

                # If no ID, construct from package info
                if not pkg_id:
                    pkg_name = pkg.get('package_name', '')
                    pkg_version = pkg.get('version', '')
                    if pkg_name and pkg_version:
                        pkg_id = f"NPM::{pkg_name}:{pkg_version}"

                if not pkg_id:
                    continue

                # Get the license from fetched data
                fetched = pkg.get('fetched_license', {})
                license_found = fetched.get('license', '')
                ...
                if license_found and license_found != 'Not found':
                    ...
                    if normalized not in self.evidence_db[pkg_id]['licenses']:
                        self.evidence_db[pkg_id]['licenses'][normalized] = {'sources': [], 'confidence': 0}
                    self.evidence_db[pkg_id]['licenses'][normalized]['sources'].append('NPM_Registry')
                    self.evidence_db[pkg_id]['licenses'][normalized]['confidence'] += 0.3

        except Exception as e:
            print(f"⚠️  Warning: Could not load npm results: {e}")
...
def main():
    ...
    parser.add_argument('--npm-json', help='npm fetch results JSON file')
    ...
    if args.npm_json:
        print("\n📦 Loading npm fetch results...")
        engine.load_npm_results(args.npm_json)
