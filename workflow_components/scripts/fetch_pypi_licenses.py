#!/usr/bin/env python3
"""
ORT Analyzer License Filter - Registry License Fetcher
Identifies packages with missing licenses and attempts to fetch them from
package registries (PyPI for Python packages, npm for JavaScript packages).

This script works as Stage 2.5 in the enhanced ORT pipeline:
Stage 1: ORT Analyzer
Stage 2: Extract Uncertain Packages
Stage 2.5: Fetch Registry Licenses (THIS SCRIPT) - PyPI + npm
...
class ORTLicenseAnalyzer:
    def __init__(self, yaml_file: str, output_dir: str = "pypi-licenses"):
        ...
        self.fetch_stats = {
            ...
            'npm_packages': 0,
            'npm_successfully_fetched': 0,
            'npm_licenses_found': 0,
            'npm_fetch_errors': 0
        }
    ...
    def fetch_npm_license(self, package_name: str, version: str) -> Dict:
        ...
        try:
            ...
            # Try 'license' field (string or {type} object)
            license_field = data.get('license', '')
            if isinstance(license_field, dict):
                license_field = license_field.get('type', '')

            # Fallback to 'licenses' array (older npm format)
            if not license_field:
                licenses_array = data.get('licenses', [])
                if licenses_array:
                    if isinstance(licenses_array[0], dict):
                        license_field = licenses_array[0].get('type', '')
                    elif isinstance(licenses_array[0], str):
                        license_field = licenses_array[0]

            # Filter out non-informative values
            if license_field and license_field.upper() in ['UNLICENSED', 'UNKNOWN', 'NONE', '']:
                license_field = ''
            if license_field and license_field.upper().startswith('SEE LICENSE IN'):
                license_field = ''
    ...
    def parse_package_id(self, pkg_id: str) -> Tuple[str, str, str]:
        ...
        # Format: "PyPI::package-name:version" or "NPM::@scope/name:version"
        if '::' in pkg_id:
            parts = pkg_id.split('::')
            ecosystem = parts[0]
            ...
            # ORT single-colon format: "NPM:@scope:name:version"
            parts = pkg_id.split(':')
            if len(parts) >= 4:
                ecosystem = parts[0]
                # Reconstruct scoped package name: @namespace/name
                namespace = parts[1]
                name = parts[2]
                version = parts[3]
                if namespace.startswith('@'):
                    return ecosystem, f"{namespace}/{name}", version
                else:
                    return ecosystem, namespace, name  # name is actually version
            elif len(parts) == 3:
                return parts[0], parts[1], parts[2]
        return '', '', ''

    def enrich_missing_licenses(self):
        """Attempt to fetch license information from package registries (PyPI + npm)."""
        print(f"\n🌐 Fetching license information from package registries...\n")
        ...
        for pkg in self.missing_licenses:
            ...
            if ecosystem == 'PyPI' and name and version:
                ...
                print(f"  → Fetching from PyPI: {name}:{version}...", end=' ')
            ...
            elif ecosystem == 'NPM' and name and version:
                self.fetch_stats['npm_packages'] += 1
                print(f"  → Fetching from npm: {name}:{version}...", end=' ')

                license_info = self.fetch_npm_license(name, version)
                pkg['fetched_license'] = license_info

                if license_info.get('success'):
                    self.fetch_stats['npm_successfully_fetched'] += 1
                    if license_info.get('license'):
                        self.fetch_stats['npm_licenses_found'] += 1
                        self.npm_fetched.append(pkg)
                        print(f"✓ Found: {license_info['license']}")
                    else:
                        self.fetch_stats['licenses_still_missing'] += 1
                        print("⚠ No license info in npm metadata")
                else:
                    self.fetch_stats['npm_fetch_errors'] += 1
                    print(f"✗ Error: {license_info.get('error', 'Unknown')}")

            else:
                self.fetch_stats['non_pypi_packages'] += 1
                pkg['fetched_license'] = {'error': f'Unsupported ecosystem ({ecosystem})', 'success': False}

        print(f"\n✓ Registry license fetch complete")

    def print_report(self):
        """Print a formatted report of packages with missing licenses."""
        print("\n" + "=" * 80)
        print("REGISTRY LICENSE FETCH REPORT")
        print("=" * 80)
        print(f"\n📊 PyPI Statistics:")
        ...
        print(f"\n📊 npm Statistics:")
        print(f"   npm packages: {self.fetch_stats['npm_packages']}")
        print(f"   Successfully fetched from npm: {self.fetch_stats['npm_successfully_fetched']}")
        print(f"   Licenses found in npm: {self.fetch_stats['npm_licenses_found']}")
        print(f"   npm fetch errors: {self.fetch_stats['npm_fetch_errors']}")
        ...
        if self.npm_fetched:
            print(f"\n✅ Packages with licenses found in npm ({len(self.npm_fetched)}):\n")
            for i, pkg in enumerate(self.npm_fetched, 1):
                print(f"{i}. {pkg['id']}")
                fetched = pkg['fetched_license']
                print(f"   License: {fetched.get('license', 'N/A')}")
                print()
    ...
    def export_npm_found(self, output_file: Optional[str] = None):
        """Export only packages with licenses found in npm."""
        if output_file is None:
            output_file = self.output_dir / "npm-licenses-found.json"
        else:
            output_file = Path(output_file)
        ...
        export_data = {
            'generated_at': datetime.now().isoformat(),
            'ort_analyzer_file': str(self.yaml_file),
            'count': len(self.npm_fetched),
            'packages': self.npm_fetched
        }

        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(export_data, f, indent=2)
        print(f"💾 npm found licenses exported to: {output_file}")
            ...
            for pkg in self.missing_licenses:
                ...
                if 'fetched_license' in pkg:
                    ...
                    if fetched.get('success') and fetched.get('license'):
                        ...
                        if ecosystem == 'NPM':
                            row['status'] = 'FOUND_IN_NPM'
                        else:
                            row['status'] = 'FOUND_IN_PYPI'
                    elif fetched.get('success'):
                        row['status'] = 'REGISTRY_NO_LICENSE'
                    else:
                        row['status'] = 'FETCH_ERROR' if ecosystem in ('PyPI', 'NPM') else 'UNSUPPORTED_ECOSYSTEM'
    ...
    def export_curation_suggestions(self, output_file: Optional[str] = None):
        ...
        for pkg in self.npm_fetched:
            fetched = pkg['fetched_license']
            if fetched.get('license'):
                curation = {
                    'id': pkg['id'],
                    'curations': {
                        'comment': f"License fetched from npm registry on {datetime.now().strftime('%Y-%m-%d')}. "
                                  f"License: {fetched['license']}. "
                                  f"⚠️ REVIEW REQUIRED - Verify from source repository before applying!",
                        'concluded_license': fetched['license'],
                        'declared_license_mapping': {
    ...
    def export_stats(self, output_file: Optional[str] = None):
        ...
        total_licenses_found = self.fetch_stats['licenses_found'] + self.fetch_stats['npm_licenses_found']

        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("Registry License Fetch Statistics\n")
            f.write("=" * 50 + "\n\n")
            ...
            if self.fetch_stats['total_missing'] > 0:
                reduction_percentage = (total_licenses_found / self.fetch_stats['total_missing']) * 100

            f.write(f"\nTotal Licenses Found (PyPI + npm): {total_licenses_found}\n")
            f.write(f"ScanCode Workload Reduction: {reduction_percentage:.1f}%\n")
    ...
    def export_to_html(self, output_file: Optional[str] = None):
        """Export beautiful HTML report of registry license fetch results."""
        if output_file is None:
        ...
        total_registry_found = self.fetch_stats['licenses_found'] + self.fetch_stats['npm_licenses_found']
        total_registry_packages = self.fetch_stats['pypi_packages'] + self.fetch_stats['npm_packages']

        reduction_percentage = 0
        if self.fetch_stats['total_missing'] > 0:
            reduction_percentage = (total_registry_found / self.fetch_stats['total_missing']) * 100

        success_rate = 0
        if total_registry_packages > 0:
            success_rate = (total_registry_found / total_registry_packages) * 100

        npm_success_rate = 0
        if self.fetch_stats['npm_packages'] > 0:
            npm_success_rate = (self.fetch_stats['npm_licenses_found'] / self.fetch_stats['npm_packages']) * 100
        ...
        for pkg in self.missing_licenses:
            ...
            if ecosystem == 'PyPI' and pkg.get('fetched_license', {}).get('license'):
                found_packages.append(pkg)
            elif ecosystem == 'NPM' and pkg.get('fetched_license', {}).get('license'):
                npm_found_packages.append(pkg)
...
<head>
    ...
    <title>Registry License Fetch Report</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <header>
            <h1>🌐 Registry License Fetch Report</h1>
            <p class="subtitle">Fast License Retrieval from Package Registries (PyPI + npm)</p>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card success">
                ...
                <div style="font-size: 0.9em; margin-top: 5px;">From {self.fetch_stats['pypi_packages']} PyPI packages</div>
            </div>

            <div class="stat-card success">
                <div class="stat-label">Licenses Found in npm</div>
                <div class="stat-number">{self.fetch_stats['npm_licenses_found']}</div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: {npm_success_rate}%"></div>
                </div>
                <div style="font-size: 0.9em; margin-top: 5px;">From {self.fetch_stats['npm_packages']} npm packages</div>
            </div>
            ...
            <div class="stat-card success">
                ...
                <div style="font-size: 0.9em; margin-top: 5px;">Overall success rate: {success_rate:.1f}%</div>
            </div>
        </div>
...
"""
        ...
        # npm Licenses Found Section
        if npm_found_packages:
            html += f"""
            <h2>✅ Licenses Found in npm ({len(npm_found_packages)} packages)</h2>
            <table>
                <thead>
                    <tr>
                        <th>Package</th>
            ...
            for pkg in npm_found_packages:
                ...
                html += f"""
                    <tr>
                        <td><span class="package-name">{name}</span></td>
                        <td><span class="version">{version}</span></td>
                        <td><span class="license-tag">{license_info}</span></td>
                        <td><span class="badge info">npm registry</span></td>
                        <td><a href="{npm_url}" target="_blank" class="link">View on npm</a></td>
                    </tr>
"""
...
"""
        ...
        if still_missing:
            ...
            <p style="color: #6b7280; margin-bottom: 20px;">These packages need ScanCode analysis or manual verification.</p>
            <table>
                <thead>
                    <tr>
                        <th>Package</th>
...
"""
            for pkg in still_missing:
                ...
                if ecosystem == 'NPM':
                    ...
                    registry_label = "View on npm"
                else:
                    registry_url = f"https://pypi.org/project/{name}/{version}/"
                    registry_label = "View on PyPI"

                html += f"""
                    <tr>
                        <td><span class="package-name">{name}</span></td>
                        <td><span class="version">{version}</span></td>
                        <td><span class="badge warning">{ecosystem}</span></td>
                        <td>{f'<a href="{homepage}" target="_blank" class="link">Homepage</a>' if homepage else '-'}</td>
                        <td><a href="{registry_url}" target="_blank" class="link">{registry_label}</a></td>
                    </tr>
"""
...
"""
        ...
        # Unsupported Ecosystem Packages Section
        if unsupported:
            html += f"""
            <h2>ℹ️ Other Ecosystem Packages ({len(unsupported)} packages)</h2>
            <p style="color: #6b7280; margin-bottom: 20px;">These packages are from unsupported ecosystems and were not checked via registry API.</p>
            <table>
                <thead>
                    <tr>
                        <th>Package ID</th>
                ...
                <tbody>
"""
            for pkg in unsupported[:20]:  # Limit to first 20 to avoid huge tables
                ecosystem, name, version = self.parse_package_id(pkg['id'])
                ...
                html += f"""
                    <tr>
                        <td colspan="3" style="text-align: center; color: #6b7280; font-style: italic;">
                            ... and {len(unsupported) - 20} more packages
                        </td>
                    </tr>
"""
...
"""
        ...
        html += f"""
            ...
            <div style="background: #f9fafb; padding: 30px; border-radius: 8px; border-left: 4px solid #667eea;">
                ...
                <ul style="line-height: 2; color: #374151;">
                    <li><strong>{self.fetch_stats['licenses_found']}</strong> licenses retrieved from PyPI</li>
                    <li><strong>{self.fetch_stats['npm_licenses_found']}</strong> licenses retrieved from npm</li>
                    <li><strong>{total_registry_found}</strong> total licenses found across all registries</li>
                ...
                <ol style="line-height: 2; color: #374151;">
                    <li><strong>Review Found Licenses:</strong> Verify accuracy from registry metadata</li>
                ...
                <div style="margin-top: 20px; padding: 15px; background: #fef3c7; border-radius: 6px; border-left: 4px solid #f59e0b;">
                    <strong>⚠️ Important:</strong> Always verify registry license information manually before applying curations.
        ...
        <footer>
            ...
            <p style="margin-top: 10px;">Registry License Fetcher - Fast, automated license retrieval from PyPI and npm</p>
...
def main():
    parser = argparse.ArgumentParser(
        description='Fetch missing license information from package registries (PyPI + npm)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage - analyze and fetch from all registries
  ...
  python fetch_pypi_licenses.py analyzer-result.yml --fetch --output-dir my-pypi-results
    ...
    parser.add_argument('--fetch', action='store_true',
                       help='Attempt to fetch missing licenses from package registries')
    ...
    print("🚀 ORT Registry License Fetcher - Stage 2.5")
    print("=" * 80)
    ...
    if args.json:
        ...
        analyzer.export_npm_found()

    if args.csv:
        analyzer.export_to_csv()

    if args.curations and (analyzer.pypi_fetched or analyzer.npm_fetched):
