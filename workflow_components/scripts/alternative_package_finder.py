...
class AlternativePackageFinder:
    ...
    def _get_npm_package_info(self, package_name: str) -> Dict:
        """Get package info from npm registry"""
        self._rate_limit('npm', 0.1)

        url = f"{self.npm_api}/{package_name}"
        response = requests.get(url, timeout=10)
        response.raise_for_status()

        return response.json()

    def _search_npm(self, search_term: str, max_results: int = 20) -> List[Dict]:
        """Search npm registry for packages"""
        self._rate_limit('npm', 0.1)

        url = f"{self.npm_api}/-/v1/search?text={search_term}&size={max_results}"
        response = requests.get(url, timeout=10)
        response.raise_for_status()

        data = response.json()
        return [obj['package'] for obj in data.get('objects', [])]

    def _extract_npm_license(self, pkg_info: Dict) -> str:
        """Extract license from npm package info"""
        # Get latest version info
        latest_tag = pkg_info.get('dist-tags', {}).get('latest', '')
        if latest_tag:
            version_data = pkg_info.get('versions', {}).get(latest_tag, {})
        else:
            version_data = pkg_info
        ...
        if license_field and license_field not in ('UNLICENSED', 'UNKNOWN', 'NONE', ''):
            return self._normalize_license(license_field)
        ...
        if licenses_array:
            if isinstance(licenses_array[0], dict):
                return self._normalize_license(licenses_array[0].get('type', ''))
            elif isinstance(licenses_array[0], str):
                return self._normalize_license(licenses_array[0])

        return 'UNKNOWN'

    def _create_npm_alternative(self,
                                pkg_info: Dict,
                                pkg_license: str,
                                approved_licenses: List[str]) -> Optional[AlternativePackage]:
        """Create AlternativePackage from npm registry info"""
        # Get latest version
        latest_tag = pkg_info.get('dist-tags', {}).get('latest', '')
        version_data = pkg_info.get('versions', {}).get(latest_tag, {}) if latest_tag else {}

        name = pkg_info.get('name', version_data.get('name', ''))
        version = latest_tag or version_data.get('version', '')
        description = pkg_info.get('description', version_data.get('description', ''))
        homepage = pkg_info.get('homepage', version_data.get('homepage', ''))

        # Get repository URL
        repository = ''
        repo_data = pkg_info.get('repository', version_data.get('repository', {}))
        ...
        # Get last updated from time field
        time_data = pkg_info.get('time', {})
        last_updated = ''
        if latest_tag and latest_tag in time_data:
            last_updated = time_data[latest_tag].split('T')[0]
        elif 'modified' in time_data:
            last_updated = time_data['modified'].split('T')[0]

        # Calculate scores
        compatibility_score = self._calculate_compatibility_score(
            pkg_license, approved_licenses
        )
        popularity_score = self._calculate_popularity_score(0, github_stars)
        maintenance_score = self._calculate_maintenance_score(last_updated)
    ...
    def _search_npm_alternatives(self,
        ...
        # Strategy 1: Get package info to understand what it does
        try:
            pkg_info = self._get_npm_package_info(package_name)
        ...
        # Build search terms from keywords + name parts
        search_terms = list(keywords[:5])
        name_clean = package_name.lstrip('@').replace('/', ' ').replace('-', ' ')
        ...
        for search_term in search_terms[:3]:
            try:
                search_results = self._search_npm(search_term, max_results=20)
                ...
                for result in search_results:
                    ...
                    # Get detailed info
                    try:
                        pkg_detail = self._get_npm_package_info(pkg_name)
                        pkg_license = self._extract_npm_license(pkg_detail)

                        # Only include if license is approved
                        if pkg_license in approved_licenses:
                            alt = self._create_npm_alternative(
                                pkg_detail, pkg_license, approved_licenses
                            )
            ...
            <tr>
                ...
                <td>
                    <div class="links">
                        ...
                        <a href="{'https://www.npmjs.com/package/' + alt.name if alt.package_type == 'NPM' else 'https://pypi.org/project/' + alt.name + '/'}" target="_blank">📚 {'npm' if alt.package_type == 'NPM' else 'PyPI'}</a>
                    </div>
                </td>
                <td>
                    <div class="stats">
