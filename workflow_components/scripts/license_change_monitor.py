...
class LicenseChangeMonitor:
    ...
    def _generate_recommendations(self,
        ...
        if pkg_type == 'NPM':
            registry_url = f"https://www.npmjs.com/package/{pkg_data['name']}"
        elif pkg_type == 'PyPI':
            registry_url = f"https://pypi.org/project/{pkg_data['name']}/"
        else:
            registry_url = f"package registry for {pkg_data['name']}"
        actions.append(f"5. Verify from package registry: {registry_url}")
